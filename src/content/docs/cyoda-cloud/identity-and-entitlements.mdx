...
<StepFlow label="Signing key rotation flow">

1. Old active key for the `client` audience
2. Issue a new key for the `client` audience
3. Sign new tokens with the new `keyId`
4. Grace period: accept old and new tokens
...
<SequenceFlow label="OIDC login and first API call">

1. **User → OIDC Provider** Login with browser
2. **OIDC Provider → User** Redirect with access token
3. **User → Cyoda Cloud** Call API with bearer token
4. **OIDC Provider → Cyoda Cloud** Serve JWKS so the signature can be validated
5. **Cyoda Cloud → User** Authorized response (user enrolled, roles applied)
