...
import EntityTree from '../../../components/EntityTree.astro';
...
<EntityTree
  label="Node tree for the example document"
  tableLegend="marks a node that is projected into its own SQL table.">

- **`$` (root node → ORGANIZATION)**
  - `organization`
    - `organization.name`
    - `organization.address[]`
    - **`organization.clients[]` (clients node → CLIENTS)**
      - `organization.clients[].name`
      - `organization.clients[].address[]`
  - **`quarterly_metrics[][]` (detached 2D array node → METRICS)**
    - `quarterly_metrics[*]` (outer arrays)
      - `quarterly_metrics[*][*]` (individual cell values)
