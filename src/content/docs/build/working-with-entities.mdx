...
The server rejects the parameter with `400` on a request that joins an open
transaction. A routed compute-node callback cannot set a deadline on a
transaction that it does not own. The same rule applies to `transactionSize` on
a delete and to `timeoutMillis` on a search.

## When a write is visible

A `2xx` response to a write means that the write is **visible to every later
read on every node**. A caller can write and then read its own data back. No
node lags another for a committed write.

The seven entity write operations do not use a `waitForConsistencyAfter`
parameter. The server accepts a request that carries it and ignores the value.

`503 STORAGE_UNAVAILABLE` reports a problem in the storage layer. It is
...
server reports two conditions per id in `deleteResult.idToError`, and does not
retry them: a version mismatch, which means the entity changed after the server
selected it, and a commit failure for the batch.
