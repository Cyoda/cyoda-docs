...
- `version`: Workflow schema `MAJOR.MINOR` version. The current version is
  `"1.2"` (cyoda-go v0.8.2); the server accepts `"1.1"` and `"1.2"` and stamps
  the current version on export. Malformed values such as `"1"` or `"1.0"`
  are rejected at import. Run `cyoda help workflows schema-version` for the
...
- declares a schema `version` outside the supported range (currently
  `"1.1"`–`"1.2"`) or malformed such as `"1"`/`"1.0"` (rejected with
