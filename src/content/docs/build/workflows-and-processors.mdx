...
it no criterion. An active workflow declared after it is unreachable, in the
same way as an automated transition declared after an unguarded one.
...
<Aside type="caution" title="Choose the selection field with care">
...
`400 WORKFLOW_FAILED`, and a loopback ends as a no-op.

A new binding can also discard a pending scheduled transition. The engine
deletes the task when the task next becomes due, and records
`SCHEDULED_TRANSITION_CANCEL` at that time, not at the write.
...
import with `400 VALIDATION_FAILED`. To negate a group, nest the group inside
the `NOT`.

`NOT` inverts the answer of its child. The engine does not replace it with the
negative form of the leaf operator, and the two forms select different
entities. `NOT($.tags[*] EQUALS "red")` fires when **no** element equals
`"red"`. `$.tags[*] NOT_EQUAL "red"` fires when **some** element is different.

`NOT` also fires over an empty list, an explicit `null` and an absent field,
because the wrapped leaf is false in all three states. See
...
- A criterion that carries an operator that no evaluator supports fails the
  save.
- A model-store failure during evaluation fails the save. A structural error on
  another condition in the same group does not hide it.
...
<Aside type="caution" title="The model governs the data a processor returns">
