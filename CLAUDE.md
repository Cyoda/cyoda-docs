...
5. `generate:config-data` (`scripts/generate-config-data.js`) — runs
   `cyoda help config all --format=json` against the pinned binary
   (located or downloaded by `scripts/lib/cyoda-binary.js`, same as
   steps 3 and 4), validates/normalizes the output, and writes the
   git-ignored `src/data/cyoda-config-all.json` consumed by
   `reference/configuration.mdx`.
6. `generate:schema-pages` (`scripts/generate-schema-pages.js`) — scans
...
7. `astro build` — produces `dist/`.
8. `export:markdown` (`scripts/export-markdown.js`) — exports cleaned
   markdown copies of each doc into `dist/markdown/` (used by the
   in-page "Copy page" button).
9. `generate:llms` (`scripts/generate-llms-txt.js`) — writes
   `dist/llms.txt` for LLM consumers, including a section that points
   agents at the static help mirror.
10. `generate:llms-full` (`scripts/generate-llms-full.js`) — writes a
    longer LLM-consumer artefact.
11. `generate:md-sitemap` (`scripts/generate-markdown-sitemap.js`).
12. `package:schemas` (`scripts/package-schemas.js`) — produces
...
- **Never hand-edit `src/data/cyoda-config-all.json`** — it is regenerated each
  build by `scripts/generate-config-data.js`, which runs
  `cyoda help config all --format=json` against the pinned binary. There is no
  tracked source file: the rendered configuration reference is the pinned
  release's own output by construction, so bumping `cyoda-go-version.json` is
  the only way to change it. (Before v0.8.3 the binary had no `config all`
  command and the generator read a hand-maintained
  `src/data/cyoda-config-all.source.json`; that file is retired.)
