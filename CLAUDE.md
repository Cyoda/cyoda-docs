...
5. `generate:config-data` (`scripts/generate-config-data.js`) — reads
   the tracked `src/data/cyoda-config-all.source.json`,
   validates/normalizes it, and writes the git-ignored
   `src/data/cyoda-config-all.json` consumed by
